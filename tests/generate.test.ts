...
test('embedData keeps $ sequences and neutralises script terminators', () => {
  ...
  assert.ok(html.includes('window.AGENTMATRIX = {'));
});
...
test('renderMatrixMarkdown links only http URLs and escapes quotes in titles', () => {
  ...
  const caps = { values: {}, groups: [{ id: 'g', name: 'G' }], capabilities: [{ id: 'x', group: 'g', name: 'X', question: 'q', rubric: 'r' }, { id: 'y', group: 'g', name: 'Y', question: 'q', rubric: 'r' }] };
  const cells: Cell[] = [
    { agent: 'a', capability: 'x', value: 'yes', quote: 'say "hi" (now)', evidence_url: 'https://a.x/p(1)', notes: '', confidence: 'high', verified: true, verified_at: '2026-09-10' },
    { agent: 'a', capability: 'y', value: 'yes', quote: 'q', evidence_url: '', notes: '', confidence: 'high', verified: false, verified_at: '' },
  ];
  const md = renderMatrixMarkdown(agents, caps, cells);
